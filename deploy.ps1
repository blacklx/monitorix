...
ENVEOF
    ...
    echo 'Note: Admin password will be generated by backend and shown in summary'
fi
"@
...
$redisPassword = (& ssh @sshArgs $Hostname "cd $RemoteDir && grep '^REDIS_PASSWORD=' .env | cut -d'=' -f2" 2>&1).Trim()
if (-not $redisPassword -or $redisPassword -match "error") {
    $redisPassword = "***"
}
# Admin password is retrieved from backend logs, not from .env
Write-Info "Retrieving admin password from backend logs..."
Start-Sleep -Seconds 5
$adminPassword = (& ssh @sshArgs $Hostname "cd $RemoteDir && docker-compose logs backend 2>/dev/null | grep -i 'Password:' | tail -1 | sed 's/.*Password: //' | tr -d '\r\n'" 2>&1).Trim()
...
Write-Info ""
Write-Info "   Redis Password:      $redisPassword"
Write-Info "   Redis Status:        Enabled (ready for caching and Celery)"
Write-Info ""
Write-Info "👤 Monitorix Admin User:"
Write-Info "   Username:            admin"
Write-Info "   Email:               admin@monitorix.local"
if ($adminPassword -and $adminPassword -ne "***" -and $adminPassword -ne "") {
    Write-Info "   Password:            $adminPassword"
}
Write-Info ""
Write-Info "⚡ Performance Features:"
Write-Info "   Redis Caching:      Enabled (improves performance)"
Write-Info "   Celery Jobs:        Enabled (background tasks for heavy operations)"
Write-Info ""
Write-Warn "⚠️  IMPORTANT: Save these credentials!"
Write-Warn "   The PostgreSQL and Redis passwords are stored in: $RemoteDir/.env"
Write-Warn "   Admin password is shown above (or in backend logs if auto-generated)"
Write-Warn "   SSH into VM to view: ssh $Hostname 'cat $RemoteDir/.env'"
Write-Info ""
Write-Info "📋 Next Steps:"
Write-Info "   1. SSH into VM: ssh $Hostname"
Write-Info "   2. Log in to the web UI: http://$vmIP:3000"
Write-Info "      Username: admin"
Write-Info "      Password: (check backend logs: docker-compose logs backend | grep Password)"
Write-Info "   3. Add Proxmox nodes via the web UI:"
Write-Info "      - Go to 'Proxmox Nodes' in the navigation menu"
Write-Info "      - Click 'Add Node' button"
Write-Info "      - Enter node details (name, URL, username, token)"
Write-Info "      - Click 'Test Connection' to verify"
Write-Info "      - Click 'Save' to add the node"
Write-Info ""
