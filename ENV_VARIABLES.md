...
4. **Proxmox**
   ...
   **Note**: Proxmox nodes are configured via the web UI, not through environment variables. See the main README for instructions.
...
# Proxmox (recommended)
# Note: Proxmox nodes are configured via the web UI, not through PROXMOX_NODES
# PROXMOX_NODES=  # Deprecated - use web UI instead
...
# Proxmox
# Note: Proxmox nodes are configured via the web UI, not through PROXMOX_NODES
# PROXMOX_NODES=node1:https://192.168.1.10:8006:user@pam:token123  # Deprecated - use web UI instead
PROXMOX_VERIFY_SSL=true
