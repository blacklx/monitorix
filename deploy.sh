...
REDIS_PASSWORD=$($SSH_CMD $HOST "cd $REMOTE_DIR && grep '^REDIS_PASSWORD=' .env | cut -d'=' -f2" 2>/dev/null || echo "***")
...
print_info ""
print_info "👤 Monitorix Admin User:"
...
print_warn "   The PostgreSQL and Redis passwords are stored in: $REMOTE_DIR/.env"
print_warn "   Admin password is shown above (or in backend logs if auto-generated)"
print_warn "   SSH into VM to view: ssh $HOST 'cat $REMOTE_DIR/.env'"
print_info ""
print_info "📋 Next Steps:"
print_info "   1. SSH into VM: ssh $HOST"
print_info "   2. Log in to the web UI: http://$VM_IP:3000"
print_info "      Username: admin"
print_info "      Password: (check backend logs: docker-compose logs backend | grep Password)"
print_info "   3. Add Proxmox nodes via the web UI:"
